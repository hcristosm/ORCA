...
[![Testes](https://img.shields.io/badge/testes-pytest-0a9edc)](#testes)
...
(feita com requisições reais, não suposição) mostrou dois problemas
intransponíveis sem contornar proteções que não me pareceu certo contornar:

1. **O download mensal do CEMADEN exige captcha** (`mapainterativo.cemaden.gov.br/download/download_form.php`)
   — não automatizável de forma honesta.
...
navegador automatizado — é o **pacote de dados históricos anuais do INMET**,
um ZIP público por ano com o CSV de cada estação automática do país,
atualizado com poucos dias de defasagem. É o que o ORCA usa hoje. Essa
substituição (CEMADEN → INMET) está documentada também no topo de
...
Qualquer UF do Brasil funciona (a camada é nacional); o projeto começou por
SP mas generaliza sem mudar código.
...
O primeiro download baixa o ZIP anual completo do Brasil (~55MB) e o mantém
em cache local (`data/inmet_<ano>.zip`) — downloads seguintes para outras UFs
do mesmo ano reaproveitam o cache.
...
Roda as duas ingestões em sequência, tolera a falha de uma fonte sem
derrubar a outra, e grava `data/ultima_atualizacao.txt` com o resultado. É o
mesmo script que o workflow [`atualizar-dados.yml`](.github/workflows/atualizar-dados.yml)
roda todo dia (cron `0 9 * * *`, mais um `workflow_dispatch` manual),
publicando os dados atualizados como artefato do GitHub Actions.
...
- **O limiar de atenção (padrão 100mm/72h) é ilustrativo.** É uma referência
  comum na literatura de risco de deslizamento, não um valor oficial
  calibrado para os setores da CPRM/SGB. O próprio dashboard avisa isso e
  deixa o valor livremente ajustável.
...
- **Sem autenticação/multiusuário** — é uma ferramenta local de portfólio,
  não um serviço multiusuário.
...
19 testes cobrindo: parsing de resposta ArcGIS REST (CPRM/SGB), paginação,
retry com backoff e fallback para cache local; parsing do CSV do INMET
(inclusive valores ausentes), leitura de estação dentro do ZIP anual; e a
lógica de cruzamento espacial (estação mais próxima) e temporal (chuva
acumulada 24h/72h, incluindo os casos sem dados na janela). Toda chamada de
rede é mockada — a suíte roda sem internet.
